...
# See https://github.com/ScaryMonster1331/SWE2/issues/585
...
if sys.version_info < PYTHON_MINIMUM_VERSION:
    msg = (
        ...
        "SWE2 requires Python 3.11 or higher."
    )
...
def get_agent_commit_hash() -> str:
    """Get the commit hash of the current SWE2 commit.
...
def get_agent_version_info() -> str:
    ...
    return f"This is SWE2 version {__version__} ({hash=}) with SWE-ReX version {rex_version} ({rex_hash=})."
...
def impose_rex_lower_bound() -> None:
    ...
    if version.parse(rex_version) < version.parse(SWEREX_RECOMMENDED_VERSION):
        ...
        get_logger("swe2", emoji="👋").warning(msg)


impose_rex_lower_bound()
get_logger("swe2", emoji="👋").info(get_agent_version_info())
