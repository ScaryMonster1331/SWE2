...
def open_pr(*, logger, token, env: SWEEnv, github_url, trajectory, _dry_run: bool = False) -> None:
    ...
    branch_name = f"swe2-fix-#{issue.number}-" + str(random.random())[2:10]
    env.communicate(
        input="git config user.email 'noemail@swe2.local' && git config user.name 'SWE2'",
    ...
    body = (
        f"This is a PR opened by AI tool [SWE Agent](https://github.com/ScaryMonster1331/SWE2/) "
    ...
    if not _dry_run:
        args = dict(
            owner=owner,
            repo=repo,
            title=f"SWE2[bot] PR to fix: {issue.title}",
            head=head,
            base=default_branch,
            body=body,
            draft=True,
...
def format_trajectory_markdown(trajectory: list[dict[str, str]], char_limit: int | None = None):
    ...
    prefix = [
        "<details>",
        "<summary>Thought process ('trajectory') of SWE2 (click to expand)</summary>",
        "",
        "",
    ]
    prefix_text = "\n".join(prefix)
