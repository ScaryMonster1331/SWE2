"""[cyan][bold]Main command line interface for SWE2.[/bold][/cyan]
...
[bold][green]run[/green][/bold] or [bold][green]r[/green][/bold]: Run swe2 on a single problem statement, for example a github issue.
[bold][green]run-batch[/green][/bold] or [bold][green]b[/green][/bold]: Run swe2 on a batch of problem statements, e.g., on SWE-Bench.
...
[bold][green]run-api[/green][/bold]: Run swe2 as a backend for a GUI
...
def main(args: list[str] | None = None):
    ...
    elif command in ["requirements", "req"]:
        from sweagent.requirements.cli import run_from_cli as requirements_main

        requirements_main(remaining_args)
